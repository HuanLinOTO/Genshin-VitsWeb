...
const unzip = (path, outpath, callback) => {
	...
	var unzip_process = spawn(
		'./7z.exe',
		['x', './tmp/' + path, '-o' + outpath],
		{
			silent: true,
		}
	);
		...
		() => {
			...
			inquirer
				.prompt([
					{
						...
						name: 'miniconda_path',
						message:
							'请输入你想要把miniconda安装到哪个目录(不知道直接回车)',
				...
				.then((answers) => {
					...
					let miniconda_install_process = spawn(
						'./tmp/miniconda_installer.exe',
						['/S', '/D=' + miniconda_path],
						{
							silent: true,
						}
...
function activate_conda(callback) {
	...
	conda.stdout.on('data', (data) => {
		...
		if (
			content.indexOf('monotonic_alignff') != -1 &&
			!ismonotonic_alignloaded
		) {
